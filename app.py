...
# =======================================================================
# VERSÃO COMPLETA E CORRIGIDA: Adicionados todos os campos do formulário
# ao banco de dados e melhorada a robustez na gravação dos dados.
# =======================================================================
...
class CadastroReurb(db.Model):
    ...
    imovel_tipo_construcao = db.Column(db.String(50), nullable=True)
    tipo_imovel = db.Column(db.String(50), nullable=True) # Alíquota
    ...
    imovel_estrutura = db.Column(db.String(50), nullable=True)
    imovel_cobertura = db.Column(db.String(50), nullable=True)
    imovel_instalacao_sanitaria = db.Column(db.String(50), nullable=True)
    imovel_forro = db.Column(db.String(50), nullable=True)
    imovel_piso = db.Column(db.String(50), nullable=True)
    imovel_portas = db.Column(db.String(50), nullable=True)
    imovel_janelas = db.Column(db.String(50), nullable=True)
    imovel_revestimento = db.Column(db.String(50), nullable=True)
...
# =======================================================================
def to_float(value):
    """Converte um valor para float de forma segura, retornando None em caso de erro."""
...
def to_int(value):
    """Converte um valor para int de forma segura, retornando None em caso de erro."""
    if value is None or value == '':
        return None
    try:
...
def redefinir_senha():
    ...
    if not all([usuario_str, senha_atual, senha_nova]):
        return jsonify({'erro': 'Todos os campos são obrigatórios'}), 400

    user = Usuario.query.filter_by(usuario=usuario_str).first()

    if not user or not user.verificar_senha(senha_atual):
        return jsonify({'erro': 'Usuário ou senha atual incorreta'}), 401
...
# ------------------- CADASTRO REURB (ROTA CORRIGIDA E AMPLIADA) -------------------
...
def cadastrar_reurb(current_user):
    ...
    try:
        novo_cadastro = CadastroReurb(
            ...
            req_regime_bens=data.get('req_regime_bens'),
            conj_nome=data.get('conj_nome'),
            conj_cpf=data.get('conj_cpf'),
            conj_rg=data.get('conj_rg'),
            conj_data_nasc=data.get('conj_data_nasc'),
            conj_nacionalidade=data.get('conj_nacionalidade'),
            conj_profissao=data.get('conj_profissao'),
            conj_telefone=data.get('conj_telefone'),
            ...
            confrontante_frente=data.get('confrontante_frente'),
            confrontante_fundo=data.get('confrontante_fundo'),
            confrontante_ld=data.get('confrontante_ld'),
            confrontante_le=data.get('confrontante_le'),
            ...
            imovel_area_construida=to_float(data.get('imovel_area_construida')),
            ...
            latitude=to_float(data.get('imovel_latitude')),
            longitude=to_float(data.get('imovel_longitude')),
            imovel_medidor_agua=data.get('imovel_medidor_agua'),
            imovel_medidor_luz=data.get('imovel_medidor_luz'),
            imovel_num_habitantes=to_int(data.get('imovel_num_habitantes')),
            imovel_estrutura=data.get('imovel_estrutura'),
            imovel_cobertura=data.get('imovel_cobertura'),
            imovel_instalacao_sanitaria=data.get('imovel_instalacao_sanitaria'),
            imovel_forro=data.get('imovel_forro'),
            imovel_piso=data.get('imovel_piso'),
            imovel_portas=data.get('imovel_portas'),
            imovel_janelas=data.get('imovel_janelas'),
            imovel_revestimento=data.get('imovel_revestimento'),
            imovel_muro=data.get('imovel_muro'),
            imovel_portoes=data.get('imovel_portoes'),
            imovel_cerca_eletrica=data.get('imovel_cerca_eletrica'),
            imovel_piscina=data.get('imovel_piscina'),
        ...
        app.logger.error(f"Dados recebidos: {data}")
...
def gerenciar_cadastro_por_id(current_user, id):
    ...
    if request.method == 'GET':
        ...
        # Garante que datas e datetimes sejam serializáveis
        for key, value in cadastro_data.items():
    ...
    if request.method == 'PUT':
        data = request.get_json()
        # O loop dinâmico já lida com todos os campos existentes no modelo
        for key, value in data.items():
            if hasattr(cadastro, key) and key != 'id':
                # Converte os tipos de dados de forma segura ao atualizar
                if key in ['latitude', 'longitude', 'imovel_medida_frente', 'imovel_medida_fundo', 'imovel_medida_ld', 'imovel_medida_le', 'imovel_area_total', 'imovel_area_construida', 'reurb_renda_familiar']:
                    setattr(cadastro, key, to_float(value))
                elif key in ['imovel_num_habitantes']:
